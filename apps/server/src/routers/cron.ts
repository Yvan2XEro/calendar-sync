...
export const cronRouter = router({
  syncGoogleCalendars: publicProcedure.input(syncInputSchema).mutation(async ({ input }) => {
    const limit = input?.limit ?? DEFAULT_EVENT_LIMIT;
    const lookaheadHours = input?.lookaheadHours ?? DEFAULT_LOOKAHEAD_HOURS;
    ...
    if (googleAccounts.length === 0 || eventsToSync.length === 0) {
      return { summary, errors } as const;
    }
      ...
      try {
        ...
        for (const row of eventsToSync) {
          ...
          try {
            ...
            summary.skipped += 1;
            const message =
              error instanceof Error ? error.message : "Failed to sync event to Google Calendar";
