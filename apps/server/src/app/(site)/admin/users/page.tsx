...
export default function AdminUsersPage() {
	...
	const listKey = React.useMemo(
		() => ["adminUsers", "list", listParams] as const,
		[listParams],
	);
			...
			<Card>
				<CardHeader className="gap-4 md:flex md:items-start md:justify-between">
					<div>
						...
						<CardDescription>
							Search, filter, and manage every user across the workspace.
						</CardDescription>
					</div>
							...
							<TableBody>
								{isLoading ? (
									...
									<TableRow>
										...
										>
											No users found. Adjust your filters or try a different
											search.
										</TableCell>
									</TableRow>
								) : (
									rows.map((item: AdminUsersListOutput["items"][number]) => {
										...
										const isBanning =
											...
											banMutation.variables?.userId === item.userId;
					...
					{listQuery.data ? (
						<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
							<div className="text-muted-foreground text-sm">
								Showing {summaryStart} to {summaryEnd} of {listQuery.data.total}{" "}
								users
							</div>
							<Pagination>
								<PaginationContent>
